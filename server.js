...
class PingPongServer {
    ...
    updateAi() {
        ...
        if (ballPosition.length !== 1) {
            target = ballPosition.findIndex(
                (item, index) =>
                    item.position.z ===
                    Math.min(...ballPosition.map((b) => b.position.z)),
            );
        }
        if (
            ballPosition[target].position.x < this.gameState.balls[target].position.z
        )
            return; //is comming
        if (ballPosition[target].z > 0 || Math.floor(Math.random() * 100) < AI_RATE)
            return; //over half and ai rate
    ...
    updatePhysics() {
        ...
        this.gameState.balls.forEach((ball) => {
            ...
            // Check scoring
            if (
                Math.abs(ball.position.z) > GAME_LENGTH / 2 ||
                ball.position.y < 0 ||
                ball.position.y > 20
            ) {
                ...
                if (
                    this.gameState.score.playerOne > GAME_SET_SCORE ||
                    this.gameState.score.playerTwo > GAME_SET_SCORE
                ) {
                    console.log("end!!");
                    this.socketSend(
                        "gameEnd",
                        `winner is ${this.gameState.score.playerOne > this.gameState.score.playerTwo
    ...
    handlePlayerInput(playerId, key, pressed) {
        const player =
            playerId === "ai" || playerId === Array.from(this.clients.keys())[1]
    ...
    socketSend(type, op = null) {
        if (type === "gameStart" || type === "gameState" || type === "score") {
            ...
            if (!this.isSinglePlayer) io.to(op).emit("data", { type });
...
io.on("connection", (socket) => {
    ...
    socket.on("keyPress", (data) => {
        if (data.key !== " ")
            ...
            const player = !data.who
                ? game.gameState.playerOne
                : game.gameState.playerTwo;
            const isCollision = game.gameState.balls.filter(
                (element) =>
                    game.isInRange(
                        Math.floor(element.position.x),
                        Math.floor(player.x),
                        10,
                    ) &&
                    game.isInRange(
                        Math.floor(element.position.z),
                        Math.floor(player.z),
                        10,
                    ),
            );
